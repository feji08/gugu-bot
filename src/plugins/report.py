...
send_all_summary = on_command("周期总结", aliases={"send_all_summary"})
...
@send_all_summary.handle()
async def handle_send_all_summary(bot: Bot, event: Event):
    # 获取考勤周期的所有日期
    today = get_current_time().date()
    dates = get_cycle_dates(get_current_time())
    dates = [date for date in dates if date.date() <= min(today, config.cycle_end.date())]
    session = Session()
    # 获取所有用户的 user_id
    user_ids = session.query(User.user_id).distinct().all()
    user_ids = [user_id[0] for user_id in user_ids]
    ...
    # **保存为图片**
    save_path = Path(f"周期打卡表格_{today}.png")
    plt.savefig(save_path, dpi=300, bbox_inches="tight", pad_inches=0.1)  # 高分辨率保存
    # plt.show()

    print(f"图片已保存到: {save_path}")
    await send_summary.send("周期总结（全体）已生成，图片正在赶来的路上，请耐心等候~")
